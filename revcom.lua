...
local hist_size = 1000 -- How many items to keep on history
...
jumpcut:setTooltip("Command history")
...
local clipboard_history = {} --If no history is saved on the system, create an empty history
...
function putOnPaste(string,key)
   if (pasteOnSelect) then
      ...
      -- if (key.alt == true or key.cmd == true) then -- If the option/alt key is active when clicking on the menu, perform a "direct paste", without changing the clipboard
